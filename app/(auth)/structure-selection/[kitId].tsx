...
import { Hammer, Minus, Plus } from "lucide-react-native";
...
export default function StructureSelectionScreen() {
  ...
  const structures = useQuery(api.structures.getStructures);
  ...
  // Suggested quantity based on panels if available
  ...
  const handleConfirmSelection = async () => {
    ...
    try {
      const selectedEntries = Object.entries(selectedQuantities).filter(
        ([, qty]) => qty > 0,
      );

      await Promise.all(
        selectedEntries.map(([structureId, quantity]) =>
          addComponent({
            kitId,
            type: "structure",
      ...
      Alert.alert(
        "¡Éxito!",
        `Estructura añadida correctamente a tu kit.`,
        [
          {
            text: "OK",
      ...
      Alert.alert("Error", "No se pudo añadir la estructura al kit.");
    }
  };
  ...
  const totals = useMemo(() => {
    ...
    const items = structures.map((structure) => {
      const quantity = selectedQuantities[structure._id] ?? 0;
      const subtotal = quantity * structure.pricePerUnit;
  ...
  }, [structures, selectedQuantities]);

  const updateQuantity = (structureId: Id<"structures">, nextQuantity: number) => {
    setSelectedQuantities((prev) => ({
      ...prev,
      [structureId]: Math.max(0, nextQuantity),
    }));
  };

  if (structures === undefined || !kit) {
    return (
      <Box className="flex-1 items-center justify-center bg-background-0">
        <LoadingAnimation size={140} text="Cargando estructuras..." />
      </Box>
    );
  }
    ...
    <Box className="flex-1 bg-background-0">
      ...
      <ScrollView contentContainerStyle={{ padding: 16 }}>
        <VStack space="xl">
          ...
          <VStack space="md">
            <Heading size="lg">Opciones de Estructura</Heading>
            ...
            {structures.map((structure) => {
              ...
              return (
                <Pressable
                  ...
                  onPress={() => {
                    if (!isSelected) {
                      updateQuantity(structure._id, 1);
                    }
                  }}
                    ...
                    >
                      ...
                      <HStack space="md" className="items-center">
                        <Button
                          ...
                          onPress={() => updateQuantity(structure._id, quantity - 1)}
                        >
                        ...
                        <VStack className="ml-auto items-end">
                          <Text size="xs" className="text-typography-400">
                            Total: ${" "}
                            {(quantity * structure.pricePerUnit)
                              .toString()
                              .replace(/\B(?=(\d{3})+(?!\d))/g, ".")}
                          </Text>
                        </VStack>
      ...
      <Box className="border-t border-outline-100 bg-white p-4 pb-8">
        <HStack className="mb-3 items-center justify-between">
          <Text size="sm" className="text-typography-500">
            Seleccionados: {totals.totalQuantity}
          </Text>
        ...
        <Button
          size="lg"
          className="w-full"
          isDisabled={totals.totalQuantity === 0}
